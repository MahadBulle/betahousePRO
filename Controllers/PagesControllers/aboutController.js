...
const GetAbout = async(req,res)=>{
    let hellGetAbout=await aboutModel.find()
    res.status(200).send(hellGetAbout);
  };
//get ended
//get ById start
const GetAboutById = async(req,res)=>{
    let hellGetAboutById=await aboutModel.findById(req.params.id)
...
const PostAbout = async (req, res, next) => {
    try {
      ...
      let aboutPosting=new aboutModel(req.body);
...
const PutAbout = async (req,res)=>{
    try {
        let AboutUpdating= await aboutModel.findByIdAndUpdate(req.params.id,req.body,{new:true});
