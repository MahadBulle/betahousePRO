...
const GetUsers = async(req,res)=>{
    let hellusers=await usersModel.find();
    res.send(hellusers);
  };
//users get ends
//user getById starts
const GetUserById = async(req,res)=>{
    let helluser=await usersModel.findById(req.params.id);
    res.send(helluser);
  };
//usersgetById ends
//signup start
const signup = async (req,res)=>{
    ...
    try {
      ...
      let signupPosting=new usersModel(req.body);
      const {username,email}= req.body
      const checkUserEmail = await usersModel.findOne({email: email})
...
//user Update start
  const userUpdate = async (req,res)=>{
    ...
    //     usersModel.password
    // );
        let UserUpdating= await usersModel.findByIdAndUpdate(req.params._id,req.body,{new:true});
       res.send({
        status:'Success',
...
//user Delete start
  const userDelete = async (req,res)=>{
    let deletingById = await usersModel.findByIdAndRemove(req.params.id)
...
//login start
  const login = async (req,res)=>{
    ...
    try {
        ...
        if(error)return res.send(error.message)
        let userPosting=new usersModel(req.body);
          let currentUserData = await usersModel.findOne({username:req.body.username})
          ...
          let token = jwt.sign({id:currentUserData._id,username:currentUserData.username},"MahadBuulle")
